...
class User {
  ...
  static async get(username) {
    ...
    const jobAppliedRes = await db.query(
      `SELECT a.job_id FROM applications AS a WHERE a.username = $1`,
      [username]);
    
    user.applications = jobAppliedRes.rows.map(a => a.job_id);
    return user;
  }
  ...
  static async apply(username, jobId){
    ...
    //add data to application table (applied)
    await db.query(
